...
백룡 AI The One은 해안·지상·공중 작전기상을 한 화면에서 확인하는 정적 웹앱입니다. 외부 AI 호출 없이 서버에서 만든 기상 JSON 캐시를 우선 사용하고, 공식 자료가 없을 때는 공개 가능한 fallback 값으로 지역 목록, 지도, 개황, 객관 지표를 표시합니다.
...
정적 사이트는 브라우저에서 운영 API 키를 직접 보유하지 않습니다. 대신 서버 캐시 작업이 `npm run update:weather-cache`를 실행해 `public/data/weather-cache.json` 형식의 파일을 생성하고, 화면은 이 JSON을 1분 간격으로 확인합니다.
...
현재 스크립트는 `WEATHER_CACHE_SOURCE_URL`이 있으면 준비된 JSON을 가져오고, 없으면 서버 환경변수에 등록된 공공 API를 직접 호출합니다. API 키가 없거나 기관 승인 전 자료는 공개 가능한 fallback 캐시와 계산 보정값으로 화면을 유지합니다. 실제 기상청·해양·천문·대기질 API를 붙일 때도 프론트엔드 코드에는 키를 넣지 않습니다.
...
갱신 주기는 값의 변동성을 기준으로 나눕니다.

- 특보/속보: 기상청 기상특보 조회서비스, 5분 간격
- 일반 실황: 초단기실황/예보, ASOS, 에어코리아, 조류/수온/조위 최신 관측, 30분 간격
- 천문/조석: 한국천문연구원 출몰시각과 국립해양조사원 조석예보, 하루 2회

추가 연동 대기 API는 다음 환경변수 이름으로 분리해 둡니다. 키 값은 GitHub 저장소에 커밋하지 않고, 서버 환경파일 또는 내부망 변환 서버 환경변수로만 관리합니다.
